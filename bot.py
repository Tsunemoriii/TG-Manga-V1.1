...
from config import FORCE_CHANNEL_ID, REQ_CHANNEL_ID, env_vars, dbname, FORCE_CHANNEL, REQ_CHANNEL
...
from pagination import Pagination, db
...
async def forcesub(app: Client, msg: Message):
    ch_join = False
    ch_link = FORCE_CHANNEL
    ch_info = await app.get_chat(FORCE_CHANNEL_ID)
    ...
    try:
        ...
        if ch_join:
            try:
                buttons = [[InlineKeyboardButton(f"• Join {ch_title} •", url=ch_link)]]
                await msg.reply(
        ...
        print(f"I'm not admin in the chat: {FORCE_CHANNEL_ID} !")
    return False
...
async def forces_sub(client: Client, message: Message):
    buttons = [[InlineKeyboardButton(text="Join Channel", url=REQ_CHANNEL)]]
    text = "You must join the channel to use the bot."
    if await db.is_req_user(message.from_user.id):
        return
    await message.reply_text(text, reply_markup=InlineKeyboardMarkup(buttons))


@bot.on_chat_join_request(filters.chat(REQ_CHANNEL_ID))
...
@bot.on_message(filters.private, group=34)
async def on_new_message_event(client: Client, message: Message):
